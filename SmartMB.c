/* $Id: SmartMB.c,v 1.6 1994/12/11 18:14:51 cwikla Exp $ */
/*
...
#define SMB(a) ((a)->smart_message_box)
#define LABEL(a) ((a)->smart_message_box.labelW)
#define CONTROL(a) ((a)->smart_message_box.controlW)
#define SEP(a) ((a)->smart_message_box.separatorW)
#define DTYPE(a) ((a)->smart_message_box.dialogType)
#define PIX(a) ((a)->smart_message_box.pixmap)
#define DPOS(a) ((a)->smart_message_box.dialogPositioning)

#define MCLASS(a) ((a)->smart_message_box_class)
...
#else
static XtProc classInit();
static XtInitProc initialize(XmSmartMessageBoxWidget _request, XmSmartMessageBoxWidget _new, String *_args, Cardinal *_numArgs);
static Boolean setValues(XmSmartMessageBoxWidget _current, XmSmartMessageBoxWidget _request, XmSmartMessageBoxWidget _new);
...
static XtWidgetProc destroy(XmSmartMessageBoxWidget _smbw);
...
static Boolean cvtStringToDialogPositioning(Display *_display, XrmValuePtr _args,
	Cardinal *_numArgs, XrmValuePtr _from, XrmValuePtr _to, XtPointer *_data);
static void addLabel(XmSmartMessageBoxWidget _smbw);
static void removeLabel(XmSmartMessageBoxWidget _smbw);
static String findFile(XmSmartMessageBoxWidget, char *_name);
static void addPositioningToShell(XmSmartMessageBoxWidget _smb);
static void removePositioningFromShell(XmSmartMessageBoxWidget _smb);
...
{
	{
		...
		(XtActionList)NULL, /* smartMBActions,										/* actions */
		(Cardinal)0, /* XtNumber(smartMBActions),											 /* num_actions */
...
XmSmartMessageBoxWidget _request;
XmSmartMessageBoxWidget _new;
String *_args;
Cardinal *_numArgs;
#else
static XtInitProc initialize(XmSmartMessageBoxWidget _request, XmSmartMessageBoxWidget _new, String *_args, Cardinal *_numArgs)
...
XmSmartMessageBoxWidget _current;
XmSmartMessageBoxWidget _request;
XmSmartMessageBoxWidget _new;
#else
static Boolean setValues(XmSmartMessageBoxWidget _current, XmSmartMessageBoxWidget _request, XmSmartMessageBoxWidget _new)
...
char *_name;
#else
static String findFile(XmSmartMessageBoxWidget _smbw, char *_name)
#endif /* _NO_PROTO */
{
	char *retFile;
...
#endif /* _NO_PROTO */
{
	...
	dwidth = DisplayWidth(XtDisplay(smb), DefaultScreen(XtDisplay(smb)));
	dheight = DisplayHeight(XtDisplay(smb), DefaultScreen(XtDisplay(smb)));

	if ((x + WIDTH(smb)) > dwidth)
		x = dwidth - (WIDTH(smb) + 5);

	if ((y + HEIGHT(smb)) > dheight)
		y = dheight - (HEIGHT(smb) + 5);
